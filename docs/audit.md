...
| Billing / subscription tiers | 🟢 **Fixed.** Real Stripe Checkout, webhook, and plan gating, wired to real usage data. The plan tiers match live Stripe Products correctly. |
...
6. ~~Billing UI~~ **Done** — plan tiers, Stripe Checkout, plan status all in `/account`; the tiers themselves are wired to live Stripe products.
...
10. ~~Real pricing tiers~~ **Done** — the `.env` has been updated with real `STRIPE_PRICE_ID`s and keys mapping to live pricing.
