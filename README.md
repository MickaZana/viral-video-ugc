...
**The review-dashboard requires `DASHBOARD_USERNAME`/`DASHBOARD_PASSWORD`** in any real deployment — it approves/rejects content before it ships, so it's never served unauthenticated (see `apps/review-dashboard/src/auth.ts`). Leave both unset only for local dev/testing; the dashboard then generates a random one-time password, prints it in a plain-text banner at startup, and also saves it to `runs/dashboard-credentials.txt` — so if you missed the terminal output, the login is still sitting in a plain text file rather than buried in a JSON log line. That file is regenerated (and the old one deleted) on every restart, and is gitignored — set real env vars for a login that survives restarts.
