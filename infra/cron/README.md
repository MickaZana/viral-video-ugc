...
5. **Higgsfield access**: closed — `packages/mcp-video-gen/src/adapters/higgsfield-rest.ts` is a direct REST client against `https://platform.higgsfield.ai`, selected automatically whenever `HIGGSFIELD_ACCESS_KEY`/`HIGGSFIELD_SECRET_KEY` are set (see `.env.example`). No MCP/Claude-session dependency for a Lambda-triggered run anymore — set those two as Secrets Manager/SSM values alongside the other vendor credentials in point 4 above, and Higgsfield works as a plain Lambda-callable REST adapter exactly like Kling/Runway/Pika. Built from Higgsfield's own docs, not yet verified against a live account.
