...
async function startServer() {
  ...
  const port = (listeningServer.address() as { port: number }).port;
...
describe("review-dashboard HTTP API", () => {
  ...
  afterEach(async () => {
    // Wait for the listening socket to close before deleting the test directory
    // or starting the next server. A synchronous close() leaves its callback and
    // any keep-alive connection live into the next test in a serial worker.
    if (server) {
  ...
  it("GET /queue rejects an invalid platform query param with 400 instead of silently passing it through", async () => {
    await insertReviewItem(makeItem({ id: "a", platform: "tiktok" }));
    await startServer();
  ...
  it("GET /stats aggregates counts by status and total estimated spend across runs", async () => {
    await insertReviewItem(makeItem({ id: "a", status: "pending" }));
    await insertReviewItem(makeItem({ id: "b", status: "approved" }));
    await insertReviewItem(makeItem({ id: "c", status: "rejected" }));
  ...
  it("GET / is the operator's page — a guest with no session and no Basic Auth gets the real 401 challenge, not a redirect to /app", async () => {
    await startServer();
    const res = await fetch(`${baseUrl}/`, { redirect: "manual" });
    expect(res.status).toBe(401);
  });

  it("GET / serves the operator queue dashboard for valid Basic Auth", async () => {
    await startServer();
    const res = await fetch(`${baseUrl}/`, { headers: AUTH_HEADER, redirect: "manual" });
    expect(res.status).toBe(200);
    expect(res.headers.get("content-type")).toContain("text/html");
    const body = await res.text();
    expect(body).toContain("queue-list");
  });
  ...
  describe("authentication", () => {
    ...
    it("GET /account/join requires no credentials — the invite link Settings' Team panel hands the owner to send a teammate must not fall through to the operator Basic Auth gate, and forwards the token into the SPA's invite mode", async () => {
      await startServer();
      const res = await fetch(`${baseUrl}/account/join?token=some-invite-token`, { redirect: "manual" });
      expect(res.status).toBe(302);
      expect(res.headers.get("location")).toBe("/app?mode=invite&token=some-invite-token");
    });
